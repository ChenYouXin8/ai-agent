...
CREATE TABLE IF NOT EXISTS chen_task_events (
    ...
    CONSTRAINT fk_task_event_task FOREIGN KEY (task_id) REFERENCES chen_tasks(task_id) ON DELETE RESTRICT
);

CREATE INDEX IF NOT EXISTS idx_chen_task_events_task_time
    ON chen_task_events(task_id, created_at);

-- 幂等迁移：老库补 seq 次序键；事件 FK 由 CASCADE 改为 RESTRICT，任务删除不再连带抹掉审计轨迹
ALTER TABLE chen_task_events ADD COLUMN IF NOT EXISTS seq BIGINT GENERATED BY DEFAULT AS IDENTITY;
ALTER TABLE chen_task_events DROP CONSTRAINT IF EXISTS fk_task_event_task;
ALTER TABLE chen_task_events ADD CONSTRAINT fk_task_event_task
    FOREIGN KEY (task_id) REFERENCES chen_tasks(task_id) ON DELETE RESTRICT;

CREATE INDEX IF NOT EXISTS idx_chen_task_steps_task
    ON chen_task_steps(task_id);

CREATE INDEX IF NOT EXISTS idx_chen_task_artifacts_task
    ON chen_task_artifacts(task_id);
