...
public class TaskRepository {
    ...
    public List<TaskEvent> findEvents(String taskId, int limit) {
        return findEvents(taskId, new TaskAuditQuery(limit, 0, Long.MAX_VALUE, List.of(), null));
    }
    ...
    public List<TaskEvent> findEvents(String taskId, TaskAuditQuery query) {
        StringBuilder sql = new StringBuilder("""
                ...
                """);
        List<Object> args = new ArrayList<>(List.of(taskId, query.from(), query.to()));
        if (!query.types().isEmpty()) {
            sql.append(" AND type IN (");
            sql.append("?, ".repeat(query.types().size() - 1)).append("?)");
            query.types().forEach(type -> args.add(type.name()));
        }
        if (query.stepId() != null && !query.stepId().isBlank()) {
            sql.append(" AND step_id = ?");
            args.add(query.stepId().trim());
        }
        sql.append(" ORDER BY created_at DESC LIMIT ?");
        args.add(query.limit());
        return jdbc.query(sql.toString(), taskEventRowMapper(), args.toArray()).stream()
