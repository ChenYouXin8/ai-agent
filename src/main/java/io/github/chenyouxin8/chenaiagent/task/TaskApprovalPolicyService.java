...
public class TaskApprovalPolicyService {
    ...
    public TaskApprovalPolicyService(
        ...
        this.requiredTypes = split(requiredTypes, true);
        this.requiredKeywords = split(requiredKeywords, false);
    }

    public ApprovalPolicyDecision evaluate(PlanStep step) {
        if (step == null) return new ApprovalPolicyDecision(false, "无有效计划步骤");
        if (step.requiresApproval()) return new ApprovalPolicyDecision(true, "Planner 明确标记为需要人工审批");
        if (!enabled) return new ApprovalPolicyDecision(false, "服务端审批策略已关闭");
        ...
        for (String keyword : requiredKeywords) {
            if (!keyword.isBlank() && searchable.contains(keyword.toLowerCase(Locale.ROOT))) {
    ...
    private List<String> split(String value, boolean uppercase) {
