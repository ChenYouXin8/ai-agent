...
public class TaskRuntimeService {
    ...
    private void createPlan(ChenTask task) {
        ...
        if (containsAny(prompt, "搜索", "研究", "资料", "市场", "新闻", "网页", "调研")) {
            addStep(task, "收集信息", "使用可用的 Web / MCP 工具获取资料");
        }
        if (containsAny(prompt, "代码", "项目", "bug", "开发", "程序", "java", "vue")) {
            addStep(task, "分析或编写代码", "使用文件和终端工具完成开发工作");
        }
