...
<template>
  <div class="cm">
    <aside class="side">
      <div class="logo"><b>✦</b><span>ChenManus <i>2.5</i></span></div>
    ...
    <main class="main">
      <header>
        <div><small>AGENT WORKSPACE</small><h1>{{ current?.title || 'ChenManus 2.5' }}</h1></div>
        <router-link to="/">返回</router-link>
      </header>

      <section v-if="!current" class="empty">
        <div>✦</div><h2>把任务交给 ChenManus</h2><p>DAG 规划 · Redis 队列 · 并行 Agent · Reviewer · 长期记忆 · 可观测指标</p>
        <div class="chips">
      ...
      <section v-else class="content">
        ...
        <article class="metrics">
          ...
          <div><small>输入</small><b>~{{ current.estimatedInputTokens || 0 }}</b><span>tokens</span></div>
          <div><small>输出</small><b>~{{ current.estimatedOutputTokens || 0 }}</b><span>tokens</span></div>
          <div><small>成本</small><b>{{ Number(current.estimatedCost || 0).toFixed(4) }}</b><span>estimated</span></div>
        </article>
        ...
        <article>
          ...
          <div v-for="step in current.steps" :key="step.stepId" class="step">
            ...
            <div class="step-body">
              ...
              <p v-if="step.durationMs" class="step-meta">{{ formatDuration(step.durationMs) }} · ~{{ step.estimatedInputTokens || 0 }} in / ~{{ step.estimatedOutputTokens || 0 }} out</p>
        ...
        <article v-if="current.artifacts?.length">
          ...
          <div v-for="artifact in current.artifacts" :key="artifact.artifactId" class="artifact">
            <span>{{ artifact.type.toUpperCase() }}</span><div><b>{{ artifact.name }}</b><p>{{ artifact.path }}</p></div>
          </div>
        </article>
