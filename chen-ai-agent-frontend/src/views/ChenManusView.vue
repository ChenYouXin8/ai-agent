...
function connectEvents(id) {
  ...
  const eventNames = [
    ...
    'tool_completed', 'tool_failed', 'artifact_created', 'review_started',
    'review_completed', 'task_paused', 'task_resumed', 'task_cancelled',
...
function eventLabel(type) {
  return {
    task_created: 'TASK', plan_created: 'PLAN', step_planned: 'PLAN STEP',
    step_started: 'STEP', step_retry: 'RETRY', step_completed: 'STEP OK',
    step_failed: 'STEP ERROR', tool_started: 'TOOL START', tool_completed: 'TOOL OK',
    tool_failed: 'TOOL ERROR', artifact_created: 'ARTIFACT', review_started: 'REVIEW',
...
<template>
  <div class="cm">
    <aside class="side">
      <div class="logo"><b>✦</b><span>ChenManus <i>2.3</i></span></div>
    ...
    <main class="main">
      <header>
        <div><small>AGENT WORKSPACE</small><h1>{{ current?.title || 'ChenManus 2.3' }}</h1></div>
        <router-link to="/">返回</router-link>
      </header>

      <section v-if="!current" class="empty">
        <div>✦</div><h2>把任务交给 ChenManus</h2><p>LLM 规划 · 并行子 Agent · Reviewer · 长期记忆 · Artifact</p>
        <div class="chips">
      ...
      <section v-else class="content">
        ...
        <article>
          ...
          <div v-for="step in current.steps" :key="step.stepId" class="step">
            ...
            <div class="step-body">
              <div class="step-head"><b>{{ step.title }}</b><span v-if="step.parallelizable">可并行</span><span v-if="step.retryCount">retry {{ step.retryCount }}</span></div>
