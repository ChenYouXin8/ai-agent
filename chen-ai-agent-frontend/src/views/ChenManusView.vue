...
async function loadTasks() {
  try {
    tasks.value = await requestJson(ENDPOINTS.tasks)
  } catch (error) {
...
async function refreshTask(id) {
  try {
    current.value = await requestJson(ENDPOINTS.task(id))
  } catch (error) {
...
function connectEvents(id) {
  ...
  const eventNames = [
    ...
    'step_retry', 'step_completed', 'step_failed', 'review_started',
    ...
    <main class="main">
      <header>
        <div>
          ...
          <h1>{{ current?.title || 'ChenManus 2.0' }}</h1>
        </div>
      ...
      <section v-else class="content">
        ...
        <article v-if="events.length">
          ...
          <div v-for="(event, index) in events" :key="`${event.timestamp}-${index}`" class="event">
        ...
        <article v-if="current.result">
          <div class="section-title"><h3>最终结果</h3></div>
