...
function pushEvent(event) {
  ...
  if (events.value.length > 160) events.value.pop()
}
...
async function action(name, options = {}) {
  ...
  try {
    await requestJson(ENDPOINTS[name](current.value.taskId, scopeQuery()), { method: 'POST', ...options })
    await Promise.all([refreshTask(current.value.taskId), loadTasks(), loadQuota()])
  } catch (error) { window.alert(error.message) }
}

async function approveTask() {
  await action('taskApprove', {
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ note: 'Workspace 人工批准' }),
  })
}

async function rejectTask() {
  const note = window.prompt('请输入驳回原因（可选）', '') ?? ''
  await action('taskReject', {
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ note }),
  })
}

function hasPendingApproval(task) {
...
function stepIcon(step) {
  ...
  if (step.approvalStatus === 'PENDING') return '⚑'
...
<template>
  <div class="cm">
    <aside class="side">
      <div class="logo"><b>✦</b><span>ChenManus <i>2.9</i></span></div>
    ...
    <main class="main">
      <header>
        <div><small>AGENT WORKSPACE · TENANT {{ tenantId }}</small><h1>{{ current?.title || 'ChenManus 2.9' }}</h1></div>
        <router-link to="/">返回</router-link>
      </header>

      <section v-if="!current" class="empty">
        <div>✦</div><h2>把任务交给 ChenManus</h2><p>DAG · Team Handoff · Redis Queue · Human Approval · Reviewer · Usage · Artifact</p>
        <div class="chips">
      ...
      <section v-else class="content">
        <div class="bar">
          ...
          <button v-if="current.status === 'WAITING_USER' && hasPendingApproval(current)" class="approve" @click="approveTask">批准执行</button>
          <button v-if="current.status === 'WAITING_USER' && hasPendingApproval(current)" class="reject" @click="rejectTask">驳回</button>
        ...
        <article v-if="current.status === 'WAITING_USER' && hasPendingApproval(current)" class="approval-banner">
          ...
          <div><b>需要人工审批</b><p>ChenManus 已暂停在有副作用的执行节点。批准后才会继续，不批准不会执行该动作。</p></div>
        </article>
        ...
        <article>
          ...
          <div v-for="step in current.steps" :key="step.stepId" class="step" :class="{ approval: step.approvalStatus === 'PENDING' }">
        ...
        <article v-if="events.length">
          <div class="section-title"><h3>实时 / 审计事件</h3><small>{{ events.length }}</small></div>
          <div v-for="(event, index) in events" :key="`${event.eventId || event.timestamp}-${index}`" class="event"><span>{{ eventLabel(event.displayType) }}</span><p>{{ event.message }}</p></div>
        </article>
