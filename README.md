...
当前能力包括 LLM Planner、DAG `dependsOn`、并行子 Agent、Team Planner、Agent Handoff、Reviewer/Repair、Redis Queue、Distributed Lock、DLQ、PostgreSQL/H2、租户隔离、OAuth2/OIDC JWT、RBAC、任务/步骤 Usage、Artifact 安全下载/预览与 `/chenmanus` Workspace。
...
JWT 的 `sub`/`user_id` 提供用户身份，`tenant_id`/`tenant` 提供租户身份；缺少租户 claim 会拒绝请求。`roles`、`realm_access.roles` 和 `permissions` 映射为 Spring Security roles。

`TENANT_ADMIN` 可以管理本租户任务，`PLATFORM_ADMIN` 可以跨租户访问任务；普通用户只能访问自己的任务。`aud` 校验在配置 `CHENMANUS_OIDC_AUDIENCE` 时开启。
...
Artifact API 只允许读取 `CHENMANUS_ARTIFACT_ALLOWED_ROOT` 下的真实文件，并校验 real path；禁止 HTTP/HTTPS 外部地址和路径穿越/软链接逃逸。
